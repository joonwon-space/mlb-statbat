...
- **Code duplication**: look for repeated patterns across similar files (API handlers, components)
- **TODO/FIXME/HACK/XXX comments**: `grep -rn "TODO\|FIXME\|HACK\|XXX" frontend/src backend/app data_pipeline --include='*.ts' --include='*.tsx' --include='*.py'`
- **NotImplementedError stubs**: `grep -rn "NotImplementedError\|raise NotImplemented" backend/app --include='*.py'`
...
- **Deprecated APIs**: grep for known deprecated patterns

### 3. Type safety

- **Frontend**: `cd frontend && npx tsc --noEmit 2>&1 | tail -30` — count type errors
- **Backend**: check for missing type annotations in service files
- **`any` usage in TS**: `grep -rn ": any\|as any" frontend/src --include='*.ts' --include='*.tsx'`

### 4. Test gaps

- **Backend**: check for test files vs source modules
- **Frontend**: check for components without tests
- **Data pipeline**: any tests at all?
...
- **Inconsistent patterns**: different patterns for same purpose
