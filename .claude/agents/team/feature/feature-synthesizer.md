...
- `backend-architect.json` — API, database, text-to-SQL design
...
## Text-to-SQL Changes
From backend architect — LLM prompt changes, schema context updates.
...
4. **Backend services**: Business logic + text-to-SQL changes
5. **Backend API**: Route handlers
6. **Backend tests**: pytest for new endpoints and services
7. **Frontend types**: TypeScript interfaces matching API schemas
8. **Frontend API**: API calls for new endpoints
...
Analysts:
  ...
  backend:  N endpoints, M DB changes, K text-to-SQL changes
