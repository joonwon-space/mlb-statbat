...
team-discover (orchestrator, you)
  ...
  ├── [parallel] security-posture       → security audit (esp. LLM SQL injection)
  ├── [parallel] perf-bottleneck        → performance analysis
  ├── [parallel] product-strategy       → roadmap & data/feature completeness
...
3. Tells it to analyze the codebase following its checklist
4. Tells it to output ONLY the JSON format specified in its agent file
5. Tells it to write its output to `docs/reviews/team-{name}.json`
...
2. Write combined results to `docs/reviews/team-analysis-latest.json`
...
Phase 1 — Specialist Analysis:
  ...
  product:       N findings + data/feature completeness snapshot
