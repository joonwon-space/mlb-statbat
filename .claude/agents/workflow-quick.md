...
- `[security]`, `[auth]`, `[sql-injection]`, `[llm-query]`, `[migration]`, `[data-integrity]`
- Any item mentioning: text_to_sql, API key, SQL injection, database schema, data pipeline

**If ANY such items are found:**
1. Print warning: "tasks.md에 보안/데이터 무결성 관련 항목이 포함되어 있습니다. `/sprint`를 사용해주세요."
...
**Re-run safety check** on the updated `docs/plan/tasks.md`. If new security/data-integrity items were discovered, STOP and suggest `/sprint`.
