...
## App Safety Rules (CRITICAL)

**Sensitive paths** — changes here get EXTRA scrutiny:
- `backend/app/text_to_sql.py` (LLM-powered SQL generation — SQL injection risk)
- `backend/app/database.py` (async DB session, connection pool)
- `backend/app/config.py` (API keys, secrets)
- `backend/app/models.py` (database schema definitions)
- `data_pipeline/**` (data ingestion — data integrity risk)

**Rules:**
- Review is NEVER skipped for bug fixes
- If fix touches sensitive paths and review returns REQUEST CHANGES → STOP and escalate to user (do NOT auto-fix)
- SQL injection bugs in text_to_sql.py are always CRITICAL severity
